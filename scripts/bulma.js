// Bulma Interactive Components Script
...
function initTabs() {
    const tabButtons = document.querySelectorAll('.bulma-tab[data-tab]');
    const tabPanels = document.querySelectorAll('.tab-panel[data-panel]');

    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            const targetTab = this.getAttribute('data-tab');
            const tabContainer = this.closest('.bulma-card');
            
            // Remove active class from all tabs in this container
            tabContainer.querySelectorAll('.bulma-tab').forEach(tab => {
                tab.classList.remove('bulma-tab-active');
            });
            
            // Add active class to clicked tab
            this.classList.add('bulma-tab-active');
...
function initAccordion() {
    const accordionHeaders = document.querySelectorAll('.bulma-accordion-header');

    accordionHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const accordionItem = this.closest('.bulma-accordion-item');
            const content = accordionItem.querySelector('.bulma-accordion-content');
            const icon = this.querySelector('.bulma-accordion-icon');
            const isActive = accordionItem.classList.contains('active');

            // Close all accordion items in the same accordion
            const accordion = accordionItem.closest('.bulma-accordion');
            accordion.querySelectorAll('.bulma-accordion-item').forEach(item => {
                if (item !== accordionItem) {
                    item.classList.remove('active');
                    const itemContent = item.querySelector('.bulma-accordion-content');
                    const itemIcon = item.querySelector('.bulma-accordion-icon');
                    if (itemContent) {
    ...
    document.querySelectorAll('.bulma-accordion-item').forEach(item => {
        const content = item.querySelector('.bulma-accordion-content');
        const icon = item.querySelector('.bulma-accordion-icon');
...
function initSwitches() {
    const switches = document.querySelectorAll('.bulma-switch');

    switches.forEach(switchEl => {
        switchEl.addEventListener('change', function() {
            // You can add custom logic here based on switch state
            const label = this.closest('.bulma-switch-label');
...
function initButtons() {
    ...
    pricingButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const planName = card.querySelector('.bulma-h3').textContent;
    ...
    const componentButtons = document.querySelectorAll('.bulma-buttons-demo button');
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (href !== '#' && href !== '') {
                ...
                if (target) {
                    const navHeight = document.querySelector('.bulma-nav').offsetHeight;
...
function initTableInteractivity() {
    const tableRows = document.querySelectorAll('.bulma-table tbody tr');
...
function initBadges() {
    const badges = document.querySelectorAll('.bulma-badge');
