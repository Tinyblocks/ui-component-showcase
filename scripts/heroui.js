// HeroUI Interactive Components Script
...
function initTabs() {
    const tabButtons = document.querySelectorAll('.heroui-tab[data-tab]');
    const tabPanels = document.querySelectorAll('.tab-panel[data-panel]');

    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            const targetTab = this.getAttribute('data-tab');
            const tabContainer = this.closest('.heroui-card');
            
            // Remove active class from all tabs in this container
            tabContainer.querySelectorAll('.heroui-tab').forEach(tab => {
                tab.classList.remove('heroui-tab-active');
            });
            
            // Add active class to clicked tab
            this.classList.add('heroui-tab-active');
...
function initAccordion() {
    const accordionHeaders = document.querySelectorAll('.heroui-accordion-header');

    accordionHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const accordionItem = this.closest('.heroui-accordion-item');
            const content = accordionItem.querySelector('.heroui-accordion-content');
            const icon = this.querySelector('.heroui-accordion-icon');
            const isActive = accordionItem.classList.contains('active');

            // Close all accordion items in the same accordion
            const accordion = accordionItem.closest('.heroui-accordion');
            accordion.querySelectorAll('.heroui-accordion-item').forEach(item => {
                if (item !== accordionItem) {
                    item.classList.remove('active');
                    const itemContent = item.querySelector('.heroui-accordion-content');
                    const itemIcon = item.querySelector('.heroui-accordion-icon');
                    if (itemContent) {
    ...
    document.querySelectorAll('.heroui-accordion-item').forEach(item => {
        const content = item.querySelector('.heroui-accordion-content');
        const icon = item.querySelector('.heroui-accordion-icon');
...
function initSwitches() {
    const switches = document.querySelectorAll('.heroui-switch');

    switches.forEach(switchEl => {
        switchEl.addEventListener('change', function() {
            // You can add custom logic here based on switch state
            const label = this.closest('.heroui-switch-label');
            ...
            // Optional: Show a toast notification (if you add toast functionality)
            // showToast(`${labelText} ${this.checked ? 'enabled' : 'disabled'}`);
        });
    });
}
...
function initButtons() {
    ...
    pricingButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const planName = card.querySelector('.heroui-h3').textContent;
    ...
    const componentButtons = document.querySelectorAll('.heroui-buttons-demo button');
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (href !== '#' && href !== '') {
                ...
                if (target) {
                    const navHeight = document.querySelector('.heroui-nav').offsetHeight;
...
function initTableInteractivity() {
    const tableRows = document.querySelectorAll('.heroui-table tbody tr');
...
function initBadges() {
    const badges = document.querySelectorAll('.heroui-badge');
...
style.textContent = `
    ...
    .heroui-table tbody tr.selected {
