// Float UI Interactive Components Script
...
function initTabs() {
    const tabButtons = document.querySelectorAll('.floatui-tab[data-tab]');
    const tabPanels = document.querySelectorAll('.tab-panel[data-panel]');

    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            const targetTab = this.getAttribute('data-tab');
            const tabContainer = this.closest('.floatui-card');
            
            // Remove active class from all tabs in this container
            tabContainer.querySelectorAll('.floatui-tab').forEach(tab => {
                tab.classList.remove('floatui-tab-active');
            });
            
            // Add active class to clicked tab
            this.classList.add('floatui-tab-active');
...
function initAccordion() {
    const accordionHeaders = document.querySelectorAll('.floatui-accordion-header');

    accordionHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const accordionItem = this.closest('.floatui-accordion-item');
            const content = accordionItem.querySelector('.floatui-accordion-content');
            const icon = this.querySelector('.floatui-accordion-icon');
            const isActive = accordionItem.classList.contains('active');

            // Close all accordion items in the same accordion
            const accordion = accordionItem.closest('.floatui-accordion');
            accordion.querySelectorAll('.floatui-accordion-item').forEach(item => {
                if (item !== accordionItem) {
                    item.classList.remove('active');
                    const itemContent = item.querySelector('.floatui-accordion-content');
                    const itemIcon = item.querySelector('.floatui-accordion-icon');
                    if (itemContent) {
    ...
    document.querySelectorAll('.floatui-accordion-item').forEach(item => {
        const content = item.querySelector('.floatui-accordion-content');
        const icon = item.querySelector('.floatui-accordion-icon');
...
function initSwitches() {
    const switches = document.querySelectorAll('.floatui-switch');

    switches.forEach(switchEl => {
        switchEl.addEventListener('change', function() {
            // You can add custom logic here based on switch state
            const label = this.closest('.floatui-switch-label');
...
function initButtons() {
    ...
    pricingButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const planName = card.querySelector('.floatui-h3').textContent;
    ...
    const componentButtons = document.querySelectorAll('.floatui-buttons-demo button');
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (href !== '#' && href !== '') {
                ...
                if (target) {
                    const navHeight = document.querySelector('.floatui-nav').offsetHeight;
...
function initTableInteractivity() {
    const tableRows = document.querySelectorAll('.floatui-table tbody tr');
...
function initBadges() {
    const badges = document.querySelectorAll('.floatui-badge');
...
style.textContent = `
    ...
    .floatui-table tbody tr.selected {
