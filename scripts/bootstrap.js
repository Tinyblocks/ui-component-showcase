// Bootstrap Interactive Components Script
...
function initTabs() {
    const tabButtons = document.querySelectorAll('.bootstrap-tab[data-tab]');
    const tabPanels = document.querySelectorAll('.tab-panel[data-panel]');

    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            const targetTab = this.getAttribute('data-tab');
            const tabContainer = this.closest('.bootstrap-card');
            
            // Remove active class from all tabs in this container
            tabContainer.querySelectorAll('.bootstrap-tab').forEach(tab => {
                tab.classList.remove('bootstrap-tab-active');
            });
            
            // Add active class to clicked tab
            this.classList.add('bootstrap-tab-active');
...
function initAccordion() {
    const accordionHeaders = document.querySelectorAll('.bootstrap-accordion-header');

    accordionHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const accordionItem = this.closest('.bootstrap-accordion-item');
            const content = accordionItem.querySelector('.bootstrap-accordion-content');
            const icon = this.querySelector('.bootstrap-accordion-icon');
            const isActive = accordionItem.classList.contains('active');

            // Close all accordion items in the same accordion
            const accordion = accordionItem.closest('.bootstrap-accordion');
            accordion.querySelectorAll('.bootstrap-accordion-item').forEach(item => {
                if (item !== accordionItem) {
                    item.classList.remove('active');
                    const itemContent = item.querySelector('.bootstrap-accordion-content');
                    const itemIcon = item.querySelector('.bootstrap-accordion-icon');
                    if (itemContent) {
    ...
    document.querySelectorAll('.bootstrap-accordion-item').forEach(item => {
        const content = item.querySelector('.bootstrap-accordion-content');
        const icon = item.querySelector('.bootstrap-accordion-icon');
...
function initSwitches() {
    const switches = document.querySelectorAll('.bootstrap-switch');

    switches.forEach(switchEl => {
        switchEl.addEventListener('change', function() {
            // You can add custom logic here based on switch state
            const label = this.closest('.bootstrap-switch-label');
...
function initButtons() {
    ...
    pricingButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const planName = card.querySelector('.bootstrap-h3').textContent;
    ...
    const componentButtons = document.querySelectorAll('.bootstrap-buttons-demo button');
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (href !== '#' && href !== '') {
                ...
                if (target) {
                    const navHeight = document.querySelector('.bootstrap-nav').offsetHeight;
...
function initTableInteractivity() {
    const tableRows = document.querySelectorAll('.bootstrap-table tbody tr');
...
function initBadges() {
    const badges = document.querySelectorAll('.bootstrap-badge');
