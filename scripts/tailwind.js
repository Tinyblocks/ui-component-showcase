// Tailwind CSS Interactive Components Script
...
function initTabs() {
    const tabButtons = document.querySelectorAll('.tailwind-tab[data-tab]');
    const tabPanels = document.querySelectorAll('.tab-panel[data-panel]');

    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            const targetTab = this.getAttribute('data-tab');
            const tabContainer = this.closest('.tailwind-card');
            
            // Remove active class from all tabs in this container
            tabContainer.querySelectorAll('.tailwind-tab').forEach(tab => {
                tab.classList.remove('tailwind-tab-active');
            });
            
            // Add active class to clicked tab
            this.classList.add('tailwind-tab-active');
...
function initAccordion() {
    const accordionHeaders = document.querySelectorAll('.tailwind-accordion-header');

    accordionHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const accordionItem = this.closest('.tailwind-accordion-item');
            const content = accordionItem.querySelector('.tailwind-accordion-content');
            const icon = this.querySelector('.tailwind-accordion-icon');
            const isActive = accordionItem.classList.contains('active');

            // Close all accordion items in the same accordion
            const accordion = accordionItem.closest('.tailwind-accordion');
            accordion.querySelectorAll('.tailwind-accordion-item').forEach(item => {
                if (item !== accordionItem) {
                    item.classList.remove('active');
                    const itemContent = item.querySelector('.tailwind-accordion-content');
                    const itemIcon = item.querySelector('.tailwind-accordion-icon');
                    if (itemContent) {
    ...
    document.querySelectorAll('.tailwind-accordion-item').forEach(item => {
        const content = item.querySelector('.tailwind-accordion-content');
        const icon = item.querySelector('.tailwind-accordion-icon');
...
function initSwitches() {
    const switches = document.querySelectorAll('.tailwind-switch');

    switches.forEach(switchEl => {
        switchEl.addEventListener('change', function() {
            // You can add custom logic here based on switch state
            const label = this.closest('.tailwind-switch-label');
...
function initButtons() {
    ...
    pricingButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const planName = card.querySelector('.tailwind-h3').textContent;
    ...
    const componentButtons = document.querySelectorAll('.tailwind-buttons-demo button');
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (href !== '#' && href !== '') {
                ...
                if (target) {
                    const navHeight = document.querySelector('.tailwind-nav').offsetHeight;
...
function initTableInteractivity() {
    const tableRows = document.querySelectorAll('.tailwind-table tbody tr');
...
function initBadges() {
    const badges = document.querySelectorAll('.tailwind-badge');
...
style.textContent = `
    ...
    .tailwind-table tbody tr.selected {
