// Magic UI Interactive Components Script
...
function initTabs() {
    const tabButtons = document.querySelectorAll('.magicui-tab[data-tab]');
    const tabPanels = document.querySelectorAll('.tab-panel[data-panel]');

    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            const targetTab = this.getAttribute('data-tab');
            const tabContainer = this.closest('.magicui-card');
            
            // Remove active class from all tabs in this container
            tabContainer.querySelectorAll('.magicui-tab').forEach(tab => {
                tab.classList.remove('magicui-tab-active');
            });
            
            // Add active class to clicked tab
            this.classList.add('magicui-tab-active');
...
function initAccordion() {
    const accordionHeaders = document.querySelectorAll('.magicui-accordion-header');

    accordionHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const accordionItem = this.closest('.magicui-accordion-item');
            const content = accordionItem.querySelector('.magicui-accordion-content');
            const icon = this.querySelector('.magicui-accordion-icon');
            const isActive = accordionItem.classList.contains('active');

            // Close all accordion items in the same accordion
            const accordion = accordionItem.closest('.magicui-accordion');
            accordion.querySelectorAll('.magicui-accordion-item').forEach(item => {
                if (item !== accordionItem) {
                    item.classList.remove('active');
                    const itemContent = item.querySelector('.magicui-accordion-content');
                    const itemIcon = item.querySelector('.magicui-accordion-icon');
                    if (itemContent) {
    ...
    document.querySelectorAll('.magicui-accordion-item').forEach(item => {
        const content = item.querySelector('.magicui-accordion-content');
        const icon = item.querySelector('.magicui-accordion-icon');
...
function initSwitches() {
    const switches = document.querySelectorAll('.magicui-switch');

    switches.forEach(switchEl => {
        switchEl.addEventListener('change', function() {
            // You can add custom logic here based on switch state
            const label = this.closest('.magicui-switch-label');
...
function initButtons() {
    ...
    pricingButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const planName = card.querySelector('.magicui-h3').textContent;
    ...
    const componentButtons = document.querySelectorAll('.magicui-buttons-demo button');
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (href !== '#' && href !== '') {
                ...
                if (target) {
                    const navHeight = document.querySelector('.magicui-nav').offsetHeight;
...
function initTableInteractivity() {
    const tableRows = document.querySelectorAll('.magicui-table tbody tr');
...
function initBadges() {
    const badges = document.querySelectorAll('.magicui-badge');
...
style.textContent = `
    ...
    .magicui-table tbody tr.selected {
