// Material-UI Interactive Components Script
...
function initTabs() {
    const tabButtons = document.querySelectorAll('.mui-tab[data-tab]');
    const tabPanels = document.querySelectorAll('.tab-panel[data-panel]');

    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            const targetTab = this.getAttribute('data-tab');
            const tabContainer = this.closest('.mui-card');
            
            // Remove active class from all tabs in this container
            tabContainer.querySelectorAll('.mui-tab').forEach(tab => {
                tab.classList.remove('mui-tab-active');
            });
...
function initAccordion() {
    const accordionHeaders = document.querySelectorAll('.mui-accordion-header');

    accordionHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const accordionItem = this.closest('.mui-accordion-item');
            const content = accordionItem.querySelector('.mui-accordion-content');
            const icon = this.querySelector('.mui-accordion-icon');
            const isActive = accordionItem.classList.contains('active');

            // Close all accordion items in the same accordion
            const accordion = accordionItem.closest('.mui-accordion');
            accordion.querySelectorAll('.mui-accordion-item').forEach(item => {
                if (item !== accordionItem) {
                    item.classList.remove('active');
                    const itemContent = item.querySelector('.mui-accordion-content');
                    const itemIcon = item.querySelector('.mui-accordion-icon');
                    if (itemContent) {
    ...
    document.querySelectorAll('.mui-accordion-item').forEach(item => {
        const content = item.querySelector('.mui-accordion-content');
        const icon = item.querySelector('.mui-accordion-icon');
...
function initSwitches() {
    const switches = document.querySelectorAll('.mui-switch');

    switches.forEach(switchEl => {
        switchEl.addEventListener('change', function() {
            // You can add custom logic here based on switch state
            const label = this.closest('.mui-switch-label');
...
function initButtons() {
    ...
    pricingButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const planName = card.querySelector('.mui-h3').textContent;
    ...
    const componentButtons = document.querySelectorAll('.mui-buttons-demo button');
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (href !== '#' && href !== '') {
                ...
                if (target) {
                    const navHeight = document.querySelector('.mui-nav').offsetHeight;
...
function initTableInteractivity() {
    const tableRows = document.querySelectorAll('.mui-table tbody tr');
...
function initBadges() {
    const badges = document.querySelectorAll('.mui-badge');
