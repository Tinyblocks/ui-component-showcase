// Chakra UI Interactive Components Script
...
function initTabs() {
    const tabButtons = document.querySelectorAll('.chakra-tab[data-tab]');
    const tabPanels = document.querySelectorAll('.tab-panel[data-panel]');

    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            const targetTab = this.getAttribute('data-tab');
            const tabContainer = this.closest('.chakra-card');
            
            // Remove active class from all tabs in this container
            tabContainer.querySelectorAll('.chakra-tab').forEach(tab => {
                tab.classList.remove('chakra-tab-active');
            });
            
            // Add active class to clicked tab
            this.classList.add('chakra-tab-active');
...
function initAccordion() {
    const accordionHeaders = document.querySelectorAll('.chakra-accordion-header');

    accordionHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const accordionItem = this.closest('.chakra-accordion-item');
            const content = accordionItem.querySelector('.chakra-accordion-content');
            const icon = this.querySelector('.chakra-accordion-icon');
            const isActive = accordionItem.classList.contains('active');

            // Close all accordion items in the same accordion
            const accordion = accordionItem.closest('.chakra-accordion');
            accordion.querySelectorAll('.chakra-accordion-item').forEach(item => {
                if (item !== accordionItem) {
                    item.classList.remove('active');
                    const itemContent = item.querySelector('.chakra-accordion-content');
                    const itemIcon = item.querySelector('.chakra-accordion-icon');
                    if (itemContent) {
    ...
    document.querySelectorAll('.chakra-accordion-item').forEach(item => {
        const content = item.querySelector('.chakra-accordion-content');
        const icon = item.querySelector('.chakra-accordion-icon');
...
function initSwitches() {
    ...
    switches.forEach(switchEl => {
        switchEl.addEventListener('change', function() {
            ...
            const label = this.closest('.chakra-switch-label');
...
function initButtons() {
    ...
    pricingButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const planName = card.querySelector('.chakra-h3').textContent;
    ...
    const componentButtons = document.querySelectorAll('.chakra-buttons-demo button');
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (href !== '#' && href !== '') {
                ...
                if (target) {
                    const navHeight = document.querySelector('.chakra-nav').offsetHeight;
...
function initTableInteractivity() {
    const tableRows = document.querySelectorAll('.chakra-table tbody tr');
...
function initBadges() {
    const badges = document.querySelectorAll('.chakra-badge');
...
style.textContent = `
    ...
    .chakra-table tbody tr.selected {
