// Aceternity UI Interactive Components Script
...
function initTabs() {
    const tabButtons = document.querySelectorAll('.aceternity-tab[data-tab]');
    const tabPanels = document.querySelectorAll('.tab-panel[data-panel]');

    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            const targetTab = this.getAttribute('data-tab');
            const tabContainer = this.closest('.aceternity-card');
            
            // Remove active class from all tabs in this container
            tabContainer.querySelectorAll('.aceternity-tab').forEach(tab => {
                tab.classList.remove('aceternity-tab-active');
            });
            
            // Add active class to clicked tab
            this.classList.add('aceternity-tab-active');
...
function initAccordion() {
    const accordionHeaders = document.querySelectorAll('.aceternity-accordion-header');

    accordionHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const accordionItem = this.closest('.aceternity-accordion-item');
            const content = accordionItem.querySelector('.aceternity-accordion-content');
            const icon = this.querySelector('.aceternity-accordion-icon');
            const isActive = accordionItem.classList.contains('active');

            // Close all accordion items in the same accordion
            const accordion = accordionItem.closest('.aceternity-accordion');
            accordion.querySelectorAll('.aceternity-accordion-item').forEach(item => {
                if (item !== accordionItem) {
                    item.classList.remove('active');
                    const itemContent = item.querySelector('.aceternity-accordion-content');
                    const itemIcon = item.querySelector('.aceternity-accordion-icon');
                    if (itemContent) {
    ...
    document.querySelectorAll('.aceternity-accordion-item').forEach(item => {
        const content = item.querySelector('.aceternity-accordion-content');
        const icon = item.querySelector('.aceternity-accordion-icon');
...
function initSwitches() {
    const switches = document.querySelectorAll('.aceternity-switch');

    switches.forEach(switchEl => {
        switchEl.addEventListener('change', function() {
            // You can add custom logic here based on switch state
            const label = this.closest('.aceternity-switch-label');
...
function initButtons() {
    ...
    pricingButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const planName = card.querySelector('.aceternity-h3').textContent;
    ...
    const componentButtons = document.querySelectorAll('.aceternity-buttons-demo button');
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (href !== '#' && href !== '') {
                ...
                if (target) {
                    const navHeight = document.querySelector('.aceternity-nav').offsetHeight;
...
function initTableInteractivity() {
    const tableRows = document.querySelectorAll('.aceternity-table tbody tr');
...
function initBadges() {
    const badges = document.querySelectorAll('.aceternity-badge');
...
style.textContent = `
    ...
    .aceternity-table tbody tr.selected {
