// Shadcn UI Interactive Components Script
...
function initTabs() {
    const tabButtons = document.querySelectorAll('.shadcn-tab[data-tab]');
    const tabPanels = document.querySelectorAll('.tab-panel[data-panel]');

    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            const targetTab = this.getAttribute('data-tab');
            const tabContainer = this.closest('.shadcn-card');
            
            // Remove active class from all tabs in this container
            tabContainer.querySelectorAll('.shadcn-tab').forEach(tab => {
                tab.classList.remove('shadcn-tab-active');
            });
            
            // Add active class to clicked tab
            this.classList.add('shadcn-tab-active');
...
function initAccordion() {
    const accordionHeaders = document.querySelectorAll('.shadcn-accordion-header');

    accordionHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const accordionItem = this.closest('.shadcn-accordion-item');
            const content = accordionItem.querySelector('.shadcn-accordion-content');
            const icon = this.querySelector('.shadcn-accordion-icon');
            const isActive = accordionItem.classList.contains('active');

            // Close all accordion items in the same accordion
            const accordion = accordionItem.closest('.shadcn-accordion');
            accordion.querySelectorAll('.shadcn-accordion-item').forEach(item => {
                if (item !== accordionItem) {
                    item.classList.remove('active');
                    const itemContent = item.querySelector('.shadcn-accordion-content');
                    const itemIcon = item.querySelector('.shadcn-accordion-icon');
                    if (itemContent) {
    ...
    document.querySelectorAll('.shadcn-accordion-item').forEach(item => {
        const content = item.querySelector('.shadcn-accordion-content');
        const icon = item.querySelector('.shadcn-accordion-icon');
...
function initSwitches() {
    const switches = document.querySelectorAll('.shadcn-switch');

    switches.forEach(switchEl => {
        switchEl.addEventListener('change', function() {
            // You can add custom logic here based on switch state
            const label = this.closest('.shadcn-switch-label');
...
function initButtons() {
    ...
    pricingButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const planName = card.querySelector('.shadcn-h3').textContent;
    ...
    const componentButtons = document.querySelectorAll('.shadcn-buttons-demo button');
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (href !== '#' && href !== '') {
                ...
                if (target) {
                    const navHeight = document.querySelector('.shadcn-nav').offsetHeight;
...
function initTableInteractivity() {
    const tableRows = document.querySelectorAll('.shadcn-table tbody tr');
...
function initBadges() {
    const badges = document.querySelectorAll('.shadcn-badge');
...
style.textContent = `
    ...
    .shadcn-table tbody tr.selected {
