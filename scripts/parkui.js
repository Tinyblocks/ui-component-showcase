// Park UI Interactive Components Script
...
function initTabs() {
    const tabButtons = document.querySelectorAll('.parkui-tab[data-tab]');
    const tabPanels = document.querySelectorAll('.tab-panel[data-panel]');

    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            const targetTab = this.getAttribute('data-tab');
            const tabContainer = this.closest('.parkui-card');
            
            // Remove active class from all tabs in this container
            tabContainer.querySelectorAll('.parkui-tab').forEach(tab => {
                tab.classList.remove('parkui-tab-active');
            });
            
            // Add active class to clicked tab
            this.classList.add('parkui-tab-active');
...
function initAccordion() {
    const accordionHeaders = document.querySelectorAll('.parkui-accordion-header');

    accordionHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const accordionItem = this.closest('.parkui-accordion-item');
            const content = accordionItem.querySelector('.parkui-accordion-content');
            const icon = this.querySelector('.parkui-accordion-icon');
            const isActive = accordionItem.classList.contains('active');

            // Close all accordion items in the same accordion
            const accordion = accordionItem.closest('.parkui-accordion');
            accordion.querySelectorAll('.parkui-accordion-item').forEach(item => {
                if (item !== accordionItem) {
                    item.classList.remove('active');
                    const itemContent = item.querySelector('.parkui-accordion-content');
                    const itemIcon = item.querySelector('.parkui-accordion-icon');
                    if (itemContent) {
    ...
    document.querySelectorAll('.parkui-accordion-item').forEach(item => {
        const content = item.querySelector('.parkui-accordion-content');
        const icon = item.querySelector('.parkui-accordion-icon');
...
function initSwitches() {
    const switches = document.querySelectorAll('.parkui-switch');

    switches.forEach(switchEl => {
        switchEl.addEventListener('change', function() {
            // You can add custom logic here based on switch state
            const label = this.closest('.parkui-switch-label');
...
function initButtons() {
    ...
    pricingButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const planName = card.querySelector('.parkui-h3').textContent;
    ...
    const componentButtons = document.querySelectorAll('.parkui-buttons-demo button');
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (href !== '#' && href !== '') {
                ...
                if (target) {
                    const navHeight = document.querySelector('.parkui-nav').offsetHeight;
...
function initTableInteractivity() {
    const tableRows = document.querySelectorAll('.parkui-table tbody tr');
...
function initBadges() {
    const badges = document.querySelectorAll('.parkui-badge');
...
style.textContent = `
    ...
    .parkui-table tbody tr.selected {
