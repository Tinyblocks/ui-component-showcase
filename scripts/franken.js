// Franken UI Interactive Components Script
...
function initTabs() {
    const tabButtons = document.querySelectorAll('.franken-tab[data-tab]');
    const tabPanels = document.querySelectorAll('.tab-panel[data-panel]');

    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            const targetTab = this.getAttribute('data-tab');
            const tabContainer = this.closest('.franken-card');
            
            // Remove active class from all tabs in this container
            tabContainer.querySelectorAll('.franken-tab').forEach(tab => {
                tab.classList.remove('franken-tab-active');
            });
            
            // Add active class to clicked tab
            this.classList.add('franken-tab-active');
...
function initAccordion() {
    const accordionHeaders = document.querySelectorAll('.franken-accordion-header');

    accordionHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const accordionItem = this.closest('.franken-accordion-item');
            const content = accordionItem.querySelector('.franken-accordion-content');
            const icon = this.querySelector('.franken-accordion-icon');
            const isActive = accordionItem.classList.contains('active');

            // Close all accordion items in the same accordion
            const accordion = accordionItem.closest('.franken-accordion');
            accordion.querySelectorAll('.franken-accordion-item').forEach(item => {
                if (item !== accordionItem) {
                    item.classList.remove('active');
                    const itemContent = item.querySelector('.franken-accordion-content');
                    const itemIcon = item.querySelector('.franken-accordion-icon');
                    if (itemContent) {
    ...
    document.querySelectorAll('.franken-accordion-item').forEach(item => {
        const content = item.querySelector('.franken-accordion-content');
        const icon = item.querySelector('.franken-accordion-icon');
...
function initSwitches() {
    const switches = document.querySelectorAll('.franken-switch');

    switches.forEach(switchEl => {
        switchEl.addEventListener('change', function() {
            // You can add custom logic here based on switch state
            const label = this.closest('.franken-switch-label');
...
function initButtons() {
    ...
    pricingButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const planName = card.querySelector('.franken-h3').textContent;
    ...
    const componentButtons = document.querySelectorAll('.franken-buttons-demo button');
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (href !== '#' && href !== '') {
                ...
                if (target) {
                    const navHeight = document.querySelector('.franken-nav').offsetHeight;
...
function initTableInteractivity() {
    const tableRows = document.querySelectorAll('.franken-table tbody tr');
...
function initBadges() {
    const badges = document.querySelectorAll('.franken-badge');
...
style.textContent = `
    ...
    .franken-table tbody tr.selected {
