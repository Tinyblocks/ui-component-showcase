// Ant Design Interactive Components Script
...
function initTabs() {
    const tabButtons = document.querySelectorAll('.antd-tab[data-tab]');
    const tabPanels = document.querySelectorAll('.tab-panel[data-panel]');

    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            const targetTab = this.getAttribute('data-tab');
            const tabContainer = this.closest('.antd-card');
            
            // Remove active class from all tabs in this container
            tabContainer.querySelectorAll('.antd-tab').forEach(tab => {
                tab.classList.remove('antd-tab-active');
            });
            
            // Add active class to clicked tab
            this.classList.add('antd-tab-active');
...
function initAccordion() {
    const accordionHeaders = document.querySelectorAll('.antd-accordion-header');

    accordionHeaders.forEach(header => {
        header.addEventListener('click', function() {
            const accordionItem = this.closest('.antd-accordion-item');
            const content = accordionItem.querySelector('.antd-accordion-content');
            const icon = this.querySelector('.antd-accordion-icon');
            const isActive = accordionItem.classList.contains('active');

            // Close all accordion items in the same accordion
            const accordion = accordionItem.closest('.antd-accordion');
            accordion.querySelectorAll('.antd-accordion-item').forEach(item => {
                if (item !== accordionItem) {
                    item.classList.remove('active');
                    const itemContent = item.querySelector('.antd-accordion-content');
                    const itemIcon = item.querySelector('.antd-accordion-icon');
                    if (itemContent) {
    ...
    document.querySelectorAll('.antd-accordion-item').forEach(item => {
        const content = item.querySelector('.antd-accordion-content');
        const icon = item.querySelector('.antd-accordion-icon');
...
function initSwitches() {
    const switches = document.querySelectorAll('.antd-switch');

    switches.forEach(switchEl => {
        switchEl.addEventListener('change', function() {
            // You can add custom logic here based on switch state
            const label = this.closest('.antd-switch-label');
...
function initButtons() {
    ...
    pricingButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const planName = card.querySelector('.antd-h3').textContent;
    ...
    const componentButtons = document.querySelectorAll('.antd-buttons-demo button');
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (href !== '#' && href !== '') {
                ...
                if (target) {
                    const navHeight = document.querySelector('.antd-nav').offsetHeight;
...
function initTableInteractivity() {
    const tableRows = document.querySelectorAll('.antd-table tbody tr');
...
function initBadges() {
    const badges = document.querySelectorAll('.antd-badge');
